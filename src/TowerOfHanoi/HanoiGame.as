package TowerOfHanoi 
{
	...
	public function HanoiGame(sprite:Group) {
		super(sprite);
			setupWorld();	
			setupDebugDraw();
			SetupHud();
	...
	public function NextLevel():void {
		if (m_Level == null ) {
			m_Level = new HanoiLevel(this);
		} else {
			Alert.show("You finished this riddle in" + (m_Level as HanoiLevel).GetScore() +" moves. Press OK to try again.", "Congratulations", Alert.OK);
	...
	override public function MouseDrag():void 	{
		if (IsGamePaused() || !IsGameRunning()) return;
		(m_Level as HanoiLevel).MouseDrag();
	}
	private function SetupHud():void {
	/*	m_HUD = new HUD();
		m_Sprite.addChild(m_HUD);
		m_HUD.y = WndHeight;*/
	...
	private function setupWorld():void {
		//backBuffer = new BitmapData(WndWidth, WndHeight, false);
		//backBuffer.fillRect(backBuffer.rect, clearColor);	//
		m_Sprite.setStyle("backgroundColor",  0xAAAAAA);
		this.m_SoundManager.AddSound("BUMP", Resources.bumpFX);
		this.m_SoundManager.AddSound("HIT", Resources.hit1FX);
	...
	protected function drawHUD():void	{
